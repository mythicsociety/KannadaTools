def tokenize_kannada_text(text, preserve_spaces=False):
    """
    Splits Kannada text into tokens based on syllable patterns. Unaltered  from the original code
    """
    ...
    consonant_syllable_pattern = rf"""
        ({C}+{N}*<{H}+(<{ZWNJ}|{ZWJ}>)*|(<{ZWNJ}|{ZWJ}>)+{H}>) +  # Core of consonant syllable
        {C}+{N}*[{A}] +                                 # Extension for Avagraha
        [< {H}+(<{ZWNJ}|{ZWJ}>)* | ({M}+{N}*){H}>]+      # Optional halant or modifiers
        {SM}*[{VD}]                                 # Optional diacritics
    """
